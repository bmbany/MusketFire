...
void Engine::CollisionManager::CheckCollision(std::list<GameObject*>* src, std::list<GameObject*>* dst)
{
	for (auto& Src : *src)
	{
		std::list<Collider*>& srcColliders = Src->GetColliders();
		for (auto Dst : *dst)
		{
			std::list<Collider*>& dstColliders = Dst->GetColliders();
			for (auto& srcCollider : srcColliders)
			{
				if (nullptr == srcCollider) continue;
				if (!srcCollider->GetActive()) continue;

				for (auto& dstCollider : dstColliders)
				{
					if (nullptr == dstCollider) continue;
					if (srcCollider == dstCollider) continue;
					if (!dstCollider->GetActive()) continue;
